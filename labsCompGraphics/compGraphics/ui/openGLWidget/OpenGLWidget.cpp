...
void OpenGLWidget::runAreaParams() const {
    if (scissorEnable)
        Lab2Primitives::makeScissors(frameWidth, frameHeight, scissorX, scissorY, scissorWidth, scissorHeight);
    if (alphaEnable) Lab2Primitives::makeAlphaFunc(alphaFunc, alphaFuncValue);
    if (blendFactorEnable) Lab2Primitives::makeBlendFunc(sFactor, dFactor);
}
