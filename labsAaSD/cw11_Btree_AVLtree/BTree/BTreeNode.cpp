...
int BTreeNode::findEqualGreaterKey(int key) {
    int index = 0;
...
int BTreeNode::getPredecessor(int index) { // Узел-предшественник должен найти крайний правый узел из левого поддерева
...
int BTreeNode::getSuccessor(int index) { // Узел-преемник находится от правого поддерева к левому
...
void BTreeNode::splitChildNode(int index, std::shared_ptr<BTreeNode> node) {
...
std::pair<const BTreeNode *, int> BTreeNode::search(int key) {
...
BTreeNode::BTreeNode(int deg, bool leaf) {
    this->deg = deg;
    this->leaf = leaf;
...
void BTreeNode::getArray(std::vector<int> &array) {
    int i;
    for (i = 0; i < size; ++i) {
        if (!leaf)
            children[i]->getArray(array);
